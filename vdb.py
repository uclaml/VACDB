...
from db import GLMDB


class LCDB(GLMDB):
    """layered contextual bandits"""

    @abstractmethod
    def get_beta_tl(self) -> float:
        raise NotImplementedError

    @abstractmethod
    def get_l_t(self) -> int:
        raise NotImplementedError
    ...
    def init_sigma(self):
        ...
        self.SigmaInv = np.zeros((L + 1, d, d), dtype=DTYPE) + np.eye(d) / self.lmbda
        # \sigma_0(alpha) is the smallest variance
        self.sigmas = self.alpha * np.power(2.0, np.arange(-1, L + 1))
        self.sigmas[0] = 0

    def update_stats(self, l, z, r, w=1):
        d = self.d
        K = self.K
    ...
    def estimate(self, r, act):
        ...
        l_t = self.get_l_t()
        self.update_stats(l_t, z, r)
        self.count_xyL[l_t, x_i, y_i] += 1

    # TODO: why decreasing lambda and kappa all the way to 0.0001 does not work?
    ...
    def summarize(self, suffix=""):
        ...
        (fig1, fig2, fig3) = fig.subfigures(3, 1, wspace=0.07, height_ratios=[2, 1, 2])
        ...
        axxxx = fig3.subplots(1, 1)
        if self.beta_hist is not None:
            for l in range(1, 5):
                axxxx.plot(self.beta_hist[l], label=f"beta_l{l}")
            axxxx.legend()
        # plt.tight_layout()
